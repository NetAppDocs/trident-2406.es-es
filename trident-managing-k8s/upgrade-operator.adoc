...
[role="lead"]
Puede actualizar Astra Trident de forma manual o mediante Helm mediante el operador Trident. Puede actualizar de una instalación del operador de Trident a otra instalación del operador de Trident o actualizar de una `tridentctl` instalación a una versión del operador de Trident. Revise link:upgrade-trident.html#select-an-upgrade-method["Seleccione un método de actualización"] antes de actualizar una instalación del operador Trident.
...
NOTE: Para actualizar desde Astra Trident que se ha instalado con el operador de espacio de nombres (de la 20,07 a la 20,10), utiliza las instrucciones de actualización de link:../earlier-versions.html["la versión instalada"]Astra Trident.
...
WARNING: Cuando actualice un clúster de Kubernetes de 1,24 a la versión 1,25 o una versión posterior que tiene Astra Trident instalado, debe actualizar los valores.yaml para establecer `excludePodSecurityPolicy` `true` o añadir `--set excludePodSecurityPolicy=true` al `helm upgrade` comando antes de actualizar el clúster.
...
+
.. Descargue la última versión de Astra Trident de link:https://github.com/NetApp/trident/releases/latest["La sección _Assets_ de GitHub"^].
...
----
. Crea un `TridentOrchestrator` CR para instalar Astra Trident.
+
[listing]
----
