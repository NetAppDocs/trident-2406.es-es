...
Además link:../trident-get-started/requirements.html["Requisitos de Astra Trident"]de , para integrar FSx para ONTAP con Astra Trident, necesita:
...
NOTE: Si especifica un rol de IAM en Astra Trident, puede omitir la especificación explícita de los `apiRegion` campos , `apiKey` y `secretKey` en Astra Trident. Para obtener más información, consulte link:../trident-use/trident-fsx-examples.html["Opciones y ejemplos de configuración de FSX para ONTAP"].
