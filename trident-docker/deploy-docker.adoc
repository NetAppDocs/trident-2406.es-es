...
.Lo que necesitará
Consulte la link:prereqs-docker.html["requisitos previos para la implementación"]. Una vez que se cumplan los requisitos previos, estará listo para poner en marcha Astra Trident.
...
. Inicie Astra Trident con el sistema de complementos gestionado. Reemplace `<version>` por la versión del plugin (xxx.xx.x) que está utilizando.
+
[listing]
----
