...
| `aggregate`  a| 
Agregado para el aprovisionamiento (opcional; si se establece, se debe asignar a la SVM). Para el `ontap-nas-flexgroup` controlador, esta opción se ignora. Todos los agregados asignados al SVM se utilizan para aprovisionar un volumen de FlexGroup.
 a| 
`aggr1`
...
| `sanType` | *Compatible solo para `ontap-san` el conductor.* Utilice para seleccionar `iscsi` iSCSI o `nvme` NVMe/TCP. | `iscsi` si está en blanco 
...
| `snapshotDir`  a| 
Opción NAS para acceder al `.snapshot` directorio, por defecto a. `false`
 a| 
`true`
...
Para obtener información sobre las instancias de FlexGroup y las cargas de trabajo adecuadas para las instancias de FlexGroup, consulte la https://www.netapp.com/pdf.html?item=/media/12385-tr4571pdf.pdf["Prácticas recomendadas y guía de implementación de los volúmenes FlexGroup de NetApp"^].
